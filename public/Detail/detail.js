...
function _amSetErr(msg) {
  const e = document.getElementById("amErr");
  if (e) {
    e.textContent = msg;
    e.className = "am-msg error";
  }
  const o = document.getElementById("amOk");
  if (o) o.textContent = "";
}
...
async function handleLogin(e) {
  ...
  try {
    ...
    _amSetErr("Login failed: " + err.message);
    if (btn) {
      btn.disabled = false;
      btn.textContent = "Log In";
    }
...
async function handleSignup(e) {
  ...
  if (password !== confirm) {
    _amSetErr("Passwords do not match!");
    return;
  }
  if (password.length < 6) {
    _amSetErr("Password must be at least 6 characters!");
    return;
  }
  const btn = e.target.querySelector('button[type="submit"]');
  if (btn) {
...
async function logout() {
  await supabaseClient.auth.signOut();
  ["userLoggedIn", "currentUser", "adminToken"].forEach((k) =>
    ...
    if (isLoggedIn && currentUser) {
      try {
        const pr = await fetch(
          API_CONFIG.getURL(
            `${API_CONFIG.endpoints.payments}/${currentUser.id}`,
          ),
        );
        if (pr.ok) {
          const purchases = await pr.json();
          isPurchased = purchases.some(
            (p) => p.resource_id === currentResource.id,
          );
        }
      } catch {}
    }
...
function displayResourceDetails(isPurchased = false) {
  ...
  document.title = currentResource.title + " - Creator Pay";
  ...
  document.getElementById("resourceDetails").innerHTML = `
    ...
    <div class="why-card">
      ...
      <div class="why-list">
        ...
        <div class="why-item"><i class="fas fa-headset"></i><span><strong>Dedicated Support</strong> — Have an issue? Email support@creatorpay.com. We respond within 2 hours.</span></div>
      </div>
    </div>`;
  ...
  document.getElementById("trustCard").innerHTML = `
    <div class="trust-card">
      <div class="trust-card-title">Why Creator Pay?</div>
...
async function handlePurchase() {
  ...
  try {
    ...
    const keyRes = await fetch(
      API_CONFIG.getURL(API_CONFIG.endpoints.paymentKey),
    );
    const { key } = await keyRes.json();

    const orderRes = await fetch(
      API_CONFIG.getURL(API_CONFIG.endpoints.createOrder),
      {
        method: "POST",
...
async function verifyPayment(response) {
  try {
    const res = await fetch(
      API_CONFIG.getURL(API_CONFIG.endpoints.verifyPayment),
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
    ...
    if (res.ok && result.success) {
      showToast(
        "Payment successful! You can now download your content.",
        "success",
      );
      loadResourceDetails();
    } else {
