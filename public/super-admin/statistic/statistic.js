...
(async function initAdmin() {
  try {
    ...
    if (typeof window.supabase !== "undefined") {
      ...
      const {
        data: { user },
      } = await _supa.auth.getUser();
      if (user?.email) {
        setAdminInfo(user.email);
        return;
      }
    }
  } catch (e) {
  ...
  if (token) {
    try {
      ...
      setAdminInfo(payload.email || "admin@creatorpay.com");
      return;
    } catch (e) {
  ...
  try {
    ...
    if (currentUser.email) {
      setAdminInfo(currentUser.email);
      return;
    }
  } catch (e) {
...
function changePageSize() {
  const val =
    (typeof _cdValues !== "undefined" ? _cdValues["pageSizeSelect"] : null) ||
...
function _renderPagination(total, pageSize, totalPages, start) {
  ...
  if (_statPageSize === "all" || totalPages <= 1) {
    el.innerHTML = "";
    return;
  }
  ...
  for (let p = 1; p <= totalPages; p++) {
    if (
      p === 1 ||
      p === totalPages ||
      (p >= _statPage - range && p <= _statPage + range)
    ) {
      pages +=
        '<button onclick="goToStatPage(' +
        p +
        ')" style="' +
        (p === _statPage ? btnActive : btnBase) +
        '">' +
        p +
        "</button>";
  ...
  el.innerHTML =
    "<div>" +
    '<span style="font-size:12px;color:#94a3b8;font-weight:500;">Showing ' +
    (start + 1) +
    "\u2013" +
    ...
    (_statPage + 1) +
    ')" ' +
    (_statPage === totalPages ? "disabled" : "") +
    ' style="' +
    btnBase +
    "opacity:" +
    (_statPage === totalPages ? ".4" : "1") +
    ';">Next \u203a</button>' +
    "</div></div>";
}
...
function renderTable(rows) {
  ...
  const slice =
    _statPageSize === "all" ? rows : rows.slice(start, start + pageSize);
  ...
  tbody.innerHTML = slice
    .map(function (p, i) {
      ...
      return (
        ...
        "</span>" +
        "</div>" +
        '<button class="view-btn" onclick="showPurchaseDetails(' +
        globalIndex +
        ')">View</button>' +
        "</div>" +
        "</td>" +
...
async function loadStats() {
  ...
  try {
    ...
    const totalRevenue = completed.reduce(
      (s, p) => s + parseFloat(p.amount || p.resource_price || 0),
      0,
    );
    ...
    ["valRevenue", "valPurchases", "valAvgOrder", "valBuyers"].forEach(
      (id, i) => setCard(id, subIds[i], "--", "Error loading"),
    );
    const tbody = document.getElementById("purchaseTbody");
    if (tbody)
      tbody.innerHTML =
        '<tr><td colspan="6"><div class="empty-state"><div class="empty-icon">\u26a0\ufe0f</div><p>' +
        err.message +
        "</p></div></td></tr>";
    const rc = document.getElementById("recordCount");
...
function showPurchaseDetails(index) {
  ...
  if (!mb) return;
  mb.innerHTML =
    '<div class="detail-row"><div class="detail-label">Buyer Email</div><div class="detail-value">' +
    (purchase.buyer_email || "--") +
    "</div></div>" +
    '<div class="detail-row"><div class="detail-label">Resource</div><div class="detail-value">' +
    (purchase.resource_title || "--") +
    "</div></div>" +
    '<div class="detail-row"><div class="detail-label">Amount</div><div class="detail-value amount">' +
    fmt(amount) +
    "</div></div>" +
    ...
    status +
    "</span></div></div>" +
    '<div class="detail-row"><div class="detail-label">Purchase Date</div><div class="detail-value">' +
    fmtDate(purchase.created_at) +
    "</div></div>" +
    '<div class="detail-row"><div class="detail-label">Purchase ID</div><div class="detail-value" style="font-size:12px;color:#64748b;">' +
    (purchase.id || "--") +
    "</div></div>";
...
function filterTable() {
  ...
  const status =
    (typeof _cdValues !== "undefined" ? _cdValues["statusFilter"] : null) ??
