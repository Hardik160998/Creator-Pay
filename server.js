...
const app = express();
const allowedOrigins = new Set(["https://creatorpay.in/"]);
...
// super-admin pages — serve from public so Vercel function bundle always contains files
["Dashboard", "statistic", "payments", "Blog"].forEach((folder) => {
  ...
  app.get(`/super-admin/${folder}`, (req, res) => res.sendFile(file));
  app.get(`/super-admin/${folder}/`, (req, res) => res.sendFile(file));
  app.get(`/super-admin/${folder}/index.html`, (req, res) =>
    res.sendFile(file),
  );
});

// super-admin static assets (js, css, images)
app.use(
  "/super-admin",
  express.static(path.join(__dirname, "public", "super-admin")),
);
...
async function ensureFileUrlColumn() {
  const { error } = await supabase
    ...
    .catch(() => {
      console.log(
        "Note: Add fileUrl column manually in Supabase if not exists",
      );
    });
}
...
app.get("/api/auth/debug-link", async (req, res) => {
  try {
    const siteUrl = "https://creatorpay.in/";
  ...
  try {
    ...
    if (linkError) {
      ...
      return res
        ...
        .json({
          success: false,
          message: "Failed to generate confirmation link: " + linkError.message,
        });
    }
    ...
    if (hashedToken) {
      ...
      return res
        .status(500)
        .json({ success: false, message: "Confirmation token not generated" });
    }
    ...
    await transporter.sendMail({
      from: `"Creator Pay" <${process.env.GMAIL_USER}>`,
      to: email,
      subject: "Confirm your Creator Pay account",
      html: `
        <div style="font-family:Inter,sans-serif;max-width:560px;margin:0 auto;padding:32px 24px;background:#f8fafc;border-radius:16px;">
          <h2 style="color:#667eea;margin:0 0 8px;">💰 Welcome to Creator Pay!</h2>
...
app.post("/api/payment/verify", async (req, res) => {
  try {
    ...
    if (!process.env.RAZORPAY_KEY_SECRET) {
      ...
      return res
        .status(500)
        .json({ success: false, error: "Payment configuration error" });
    }
    ...
    if (insertError) {
      ...
      if (insertError.message.includes("foreign key constraint")) {
        return res.status(500).json({
          success: false,
          error:
            "Database constraint error. Please ensure the payments table is properly configured.",
...
app.get("/api/statistics/purchases/:userEmail", async (req, res) => {
  try {
    ...
    const relevantPayments = payments.filter((p) =>
      userResourceIds.includes(p.resource_id),
    );
    ...
    relevantPayments.forEach((payment) => {
      ...
      if (
        payment.created_at &&
        (!userStats[buyerEmail].created_at ||
          new Date(payment.created_at) >
            new Date(userStats[buyerEmail].created_at))
      ) {
...
app.get("/api/admin/dashboard-stats", async (req, res) => {
  try {
    ...
    const [paymentsResult, resourcesResult, withdrawalsResult] =
      await Promise.all([
        supabase
          .from("payments")
          .select("*")
    ...
    const allWithdrawals = withdrawals.filter(
      (w) =>
        w.status === "approved" ||
        w.status === "completed" ||
        w.status === "pending",
...
app.post("/api/withdrawals", async (req, res) => {
  try {
    ...
    if (!user_email || !amount)
      return res
        .status(400)
        .json({ error: "user_email and amount are required" });
    const { data, error } = await supabase
      .from("withdrawals")
      .insert([
        {
    ...
    if (error) {
      ...
      if (error.message.includes('relation "withdrawals" does not exist')) {
        return res
          .status(500)
          .json({
            error: "Withdrawals table not found. Please create it in Supabase.",
          });
      }
      return res.status(500).json({ error: error.message });
    }
...
app.post("/api/contact", async (req, res) => {
  try {
    ...
    await transporter.sendMail({
      from: `"Creator Pay Contact" <${process.env.GMAIL_USER}>`,
      to: "hardikkotadiya90@gmail.com",
      replyTo: email,
      subject: `[Creator Pay Contact] ${subject}`,
      html: `
        <div style="font-family:Inter,sans-serif;max-width:600px;margin:0 auto;padding:24px;background:#f8fafc;border-radius:12px;">
          ...
          <p style="margin-top:16px;font-size:12px;color:#94a3b8;">Sent from Creator Pay Contact Form</p>
        </div>`,
    });
...
app.get("/admin/purchases", (req, res) => {
  res.sendFile(
    path.join(__dirname, "Creator Pay Admin", "Purchases", "index.html"),
  );
});
...
app.get("/admin/Resources", (req, res) => {
  res.sendFile(
    path.join(__dirname, "public", "admin", "Resources", "index.html"),
  );
});
app.get("/admin/Resources/", (req, res) => {
  res.sendFile(
...
app.get("/admin/Statistics", (req, res) => {
  res.sendFile(
    path.join(__dirname, "public", "admin", "Statistics", "index.html"),
  );
});
app.get("/admin/Statistics/", (req, res) => {
  res.sendFile(
...
app.get("/admin/Profile", (req, res) => {
  res.sendFile(
    path.join(__dirname, "public", "admin", "Profile", "index.html"),
  );
});
app.get("/admin/Profile/", (req, res) => {
  res.sendFile(
...
app.get("/admin/Withdrawal", (req, res) => {
  res.sendFile(
    path.join(__dirname, "public", "admin", "Withdrawal", "index.html"),
  );
});
app.get("/admin/Withdrawal/", (req, res) => {
  res.sendFile(
